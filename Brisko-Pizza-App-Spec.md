# Brisko Pizza — Flutter + Firebase (Auth + RTDB) + PHP Admin Backend
...
- **Two separate pieces, two separate stacks:**
  1. **Customer App (Phase 1)** — pure **Flutter + Firebase**. Talks **directly** to Firebase Authentication and Firebase Realtime Database using the `google-services.json` config file. **No PHP, no custom server, no domain involved at all** for the customer app.
  2. **Admin Panel (Phase 2)** — a Flutter Web app (deployed free on Firebase Hosting) for the UI, backed by a **PHP REST API** (hosted separately, on any cheap/free PHP hosting) for the handful of operations that need server-side trust: image upload, order status changes + push notifications, loyalty point crediting, broadcast notifications, payment webhook handling, and reports.
- **Auth Method:** Email + Password only (no phone OTP login, no social login for v1)
...
| Firebase (Spark/free plan) | Authentication (Email/Password), Realtime Database (RTDB), Cloud Messaging (FCM), Analytics, Crashlytics |
| **Customer App connection** | **Direct** — Flutter talks straight to Firebase via `google-services.json`. No backend server in the loop for browsing, cart, placing orders, or viewing status. |
| **Admin-only backend** | **PHP REST API**, used exclusively by the Admin Panel and by the payment gateway's webhook — never called by the customer app |
| Backend ↔ RTDB link (admin side only) | PHP talks to Realtime Database via its native REST API (`https://<project>-default-rtdb.<region>.firebasedatabase.app/<path>.json`), authenticated with a Firebase service-account OAuth2 token |
...
lib/
 ...
 │    └── services/                # firebase_service (Auth+RTDB), location_service, payment_service, notification_service
 ├── features/
...
├── users
│   └── {uid}
│       ├── name, email, phone, createdAt, defaultAddressId
...
## 6. Feature Modules (Phase 1 — all direct Flutter ↔ Firebase, no server involved)

### 6.1 Login & Account
- Register (name, email, password, phone), Login, Forgot password (Firebase reset email)
...
backend/
 ├── config/
 │    ├── firebase-service-account.json   # keep OUTSIDE web root / .htaccess-protected
 │    ├── config.php                       # ImgBB key, Razorpay keys, RTDB base URL
 │    └── rtdb.php
 ├── lib/
 ...
 │    └── auth_middleware.php            # verifies the admin's Firebase ID token on every call
 └── cron/
...
- Phone/OTP login and social login
...
1. Flutter project setup + Firebase integration (Auth + Realtime Database, Spark plan)
2. Theme, colors, fonts, shared widgets, app icon, splash screen
3. Auth flow (register/login/forgot password) + Profile
...
20. PHP backend skeleton: `RtdbClient.php`, `auth_middleware.php`, `config.php`
