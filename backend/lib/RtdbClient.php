...
class RtdbClient
{
    ...
    {
        $url = rtrim($this->config['rtdb_base_url'], '/') . '/' . ltrim($path, '/') . '.json';
    ...
    private function accessToken(): ?string
    {
        ...
        if (!is_file($file)) {
            throw new RuntimeException('Firebase service account file is missing. Copy firebase-service-account.json.example to firebase-service-account.json.');
        }
        ...
        $claim = $this->b64(json_encode([
            'iss' => $sa['client_email'],
            'scope' => 'https://www.googleapis.com/auth/firebase.database https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/identitytoolkit https://www.googleapis.com/auth/firebase.messaging',
        ...
        if (!$this->token) {
            throw new RuntimeException('Failed to obtain Google access token for RTDB.');
        }
        return $this->token;
    }
